...
def remove_storage_account(module, creds):
...
def create_storage_account(module, creds):
    storage_client = azure.mgmt.storage.StorageManagementClient(creds)
    ...
    result = storage_client.storage_accounts.get_properties(
        module.params.get('group_name'),
        module.params.get('storage_name')
    )

    return (True, azure_object_to_json(result.storage_account))
...
def main():
    ...
    if module.params.get('state') == 'absent':
        (changed, storage_account) = remove_storage_account(module, creds)
    elif module.params.get('state') == 'present':
        (changed, storage_account) = create_storage_account(module, creds)

    module.exit_json(changed=changed, storage_account=storage_account)
